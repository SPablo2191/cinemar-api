...
def add_reservations():
    ...
    newTupleReservation = newReservation.__iter__(newReservation.idUsuario, newReservation.idFuncion,
                                                  newReservation.idDescuento, newReservation.fechaRegistro, newReservation.total, newReservation.estado)
    ...
    for seat in data['seats']:
        ...
        newTuple = newDetail.__iter__(
            idReservation, newDetail.idButaca, newDetail.estado)
...
def get_array_json(rows):
    array = []
    for row in rows:
        item = Reserva(row[1], row[2], row[3], row[5],
                       bool(row[6]), row[4], row[0]).__dict__
        array.append(item)
    return array
